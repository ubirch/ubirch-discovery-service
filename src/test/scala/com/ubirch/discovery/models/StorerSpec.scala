...
class StorerSpec extends TestBase {
  ...
  feature("produce correct gremlin queries") {
    ...
    scenario("getUpdateOrCreateVertices should work for two vertex") {
      ...
      val expectedTraversal1 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature1)])], [HasStep([hash.eq($hash1)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label1]})]]), AggregateGlobalStep($stepLabelValue1), AddPropertyStep({value=[$timestamp1], key=[timestamp]}), AddPropertyStep({value=[$signature1], key=[signature]}), AddPropertyStep({value=[$hash1], key=[hash]}), GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature2)])], [HasStep([hash.eq($hash2)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label2]})]]), AggregateGlobalStep($stepLabelValue2), AddPropertyStep({value=[$timestamp2], key=[timestamp]}), AddPropertyStep({value=[$signature2], key=[signature]}), AddPropertyStep({value=[$hash2], key=[hash]}), SelectStep(last,[$stepLabelValue1, $stepLabelValue2])]"""
      val expectedTraversal2 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature1)])], [HasStep([hash.eq($hash1)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label1]})]]), AggregateGlobalStep($stepLabelValue1), AddPropertyStep({key=[timestamp], value=[$timestamp1]}), AddPropertyStep({key=[signature], value=[$signature1]}), AddPropertyStep({key=[hash], value=[$hash1]}), GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature2)])], [HasStep([hash.eq($hash2)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label2]})]]), AggregateGlobalStep($stepLabelValue2), AddPropertyStep({key=[timestamp], value=[$timestamp2]}), AddPropertyStep({key=[signature], value=[$signature2]}), AddPropertyStep({key=[hash], value=[$hash2]}), SelectStep(last,[$stepLabelValue1, $stepLabelValue2])]"""
      (res._1.toString() == expectedTraversal1 || res._1.toString() == expectedTraversal2) shouldBe true
    }
    ...
    scenario("getUpdateOrCreateVertices should work for n vertice") {
      ...
      def generateShouldBeStringForOneVertexOneWay(i: Int) = {
        s"""GraphStep(vertex,[]), OrStep([[HasStep([hash.eq($i)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep(${res._2.verticeAndStep.toList.find(v => v._2.properties.head.value.toString == i.toString).get._1.name}), AddPropertyStep({key=[hash], value=[$i]}), """
      }
      def generateShouldBeStringForOneVertexOtherWay(i: Int) = {
        s"""GraphStep(vertex,[]), OrStep([[HasStep([hash.eq($i)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep(${res._2.verticeAndStep.toList.find(v => v._2.properties.head.value.toString == i.toString).get._1.name}), AddPropertyStep({value=[$i], key=[hash]}), """
      }
      val expectedString1 = {
  ...
  feature("concurrency") {
    scenario("test creation vertices") {
      ...
      val jgs = Injector.get[DefaultJanusgraphStorer]
      val gc = Injector.get[GremlinConnector]
      val lock = Injector.get[Lock]
      // wait to connect to Redis
      waitUntilRedisStart(lock)
      implicit val propSet: Set[Property] = KafkaElements.propertiesToIterate
    ...
    scenario("create edge should work") {
      ...
      val mapSlaveTrees = scala.collection.mutable.Map[VertexCore, List[VertexCore]]()
      for (_ <- 1 to 100) {
      ...
      Future(ds.store(allRelations)).onComplete(_ => logger.info("finished 1"))
      Future(ds.store(allRelations)).onComplete(_ => logger.info("finished 2"))
    ...
    scenario("concrete case") {
      ...
      val lock = Injector.get[Lock]
      // wait to connect to Redis
      waitUntilRedisStart(lock, 2.seconds)
