...
class StorerSpec extends TestBase {
  ...
  feature("produce correct gremlin queries") {
    ...
    scenario("getUpdateOrCreateVertices should work for one vertex") {
      ...
      val expectedTraversal1 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature)])], [HasStep([hash.eq($hash)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep($stepLabelValue,null), AddPropertyStep({value=[$timestamp], key=[timestamp]}), AddPropertyStep({value=[$signature], key=[signature]}), AddPropertyStep({value=[$hash], key=[hash]}), SelectOneStep(last,$stepLabelValue,null)]"""
      val expectedTraversal2 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature)])], [HasStep([hash.eq($hash)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep($stepLabelValue,null), AddPropertyStep({key=[timestamp], value=[$timestamp]}), AddPropertyStep({key=[signature], value=[$signature]}), AddPropertyStep({key=[hash], value=[$hash]}), SelectOneStep(last,$stepLabelValue,null)]"""
    ...
    scenario("getUpdateOrCreateVertices should work for two vertex") {
      ...
      val expectedTraversal1 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature1)])], [HasStep([hash.eq($hash1)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label1]})]]), AggregateGlobalStep($stepLabelValue1,null), AddPropertyStep({value=[$timestamp1], key=[timestamp]}), AddPropertyStep({value=[$signature1], key=[signature]}), AddPropertyStep({value=[$hash1], key=[hash]}), GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature2)])], [HasStep([hash.eq($hash2)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label2]})]]), AggregateGlobalStep($stepLabelValue2,null), AddPropertyStep({value=[$timestamp2], key=[timestamp]}), AddPropertyStep({value=[$signature2], key=[signature]}), AddPropertyStep({value=[$hash2], key=[hash]}), SelectStep(last,[$stepLabelValue1, $stepLabelValue2])]"""
      val expectedTraversal2 = s"""[GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature1)])], [HasStep([hash.eq($hash1)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label1]})]]), AggregateGlobalStep($stepLabelValue1,null), AddPropertyStep({key=[timestamp], value=[$timestamp1]}), AddPropertyStep({key=[signature], value=[$signature1]}), AddPropertyStep({key=[hash], value=[$hash1]}), GraphStep(vertex,[]), OrStep([[HasStep([signature.eq($signature2)])], [HasStep([hash.eq($hash2)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label2]})]]), AggregateGlobalStep($stepLabelValue2,null), AddPropertyStep({key=[timestamp], value=[$timestamp2]}), AddPropertyStep({key=[signature], value=[$signature2]}), AddPropertyStep({key=[hash], value=[$hash2]}), SelectStep(last,[$stepLabelValue1, $stepLabelValue2])]"""
      (res._1.toString() == expectedTraversal1 || res._1.toString() == expectedTraversal2) shouldBe true
    }
    ...
    scenario("getUpdateOrCreateVertices should work for n vertice") {
      ...
      def generateShouldBeStringForOneVertexOneWay(i: Int) = {
        s"""GraphStep(vertex,[]), OrStep([[HasStep([hash.eq($i)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep(${res._2.verticeAndStep.toList.find(v => v._2.properties.head.value.toString == i.toString).get._1.name},null), AddPropertyStep({key=[hash], value=[$i]}), """
      }
      def generateShouldBeStringForOneVertexOtherWay(i: Int) = {
        s"""GraphStep(vertex,[]), OrStep([[HasStep([hash.eq($i)])]]), FoldStep, CoalesceStep([[UnfoldStep], [AddVertexStep({label=[$label]})]]), AggregateGlobalStep(${res._2.verticeAndStep.toList.find(v => v._2.properties.head.value.toString == i.toString).get._1.name},null), AddPropertyStep({value=[$i], key=[hash]}), """
      }
      val expectedString1 = {
  ...
  ignore("concurrency") {
    ...
    scenario("try replicate bug") {
      ...
      val v = gc.g.V().l()
      logger.debug(s"vertex size: ${v.size}")
      logger.debug(s"""vertices: ${v.mkString(", ")}""")
      logger.debug("Count of vertices: " + count)
