...
if [ "$1" == 'janusgraph' ]; then
  ...
  # wait for storage
   if ! [ -z "${JANUS_STORAGE_TIMEOUT:-}" ]; then
    ...
    echo "graph = JanusGraphFactory.open('${JANUS_CONFIG_DIR}/janusgraph-cql-server.properties')" > $F
    timeout ${JANUS_STORAGE_TIMEOUT}s bash -c \
      "until bin/gremlin.sh -e $F > /dev/null 2>&1; do echo \"waiting for storage...\"; sleep 5; done"
   fi
  cat ${JANUS_CONFIG_DIR}/gremlin-server.yaml
  echo "------------------------------------------------------------"
  cat ${JANUS_HOME}/bin/gremlin-server.sh
  echo "------------------------------------------------------------"
  echo "janusgraph.properties:"
  cat ${JANUS_CONFIG_DIR}/janusgraph.properties
